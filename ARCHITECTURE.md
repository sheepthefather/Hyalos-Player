...
- **转场是 200ms 的交叉淡化，不是 Nav3 默认的 700ms。** `NavDisplay` 的 `transitionSpec` 与 `popTransitionSpec` 都显式传了（`screenFade()`），形状照抄框架默认——两个屏互相溶解、没有位移——只把时长压到约三分之一。700 这个数是从 Nav3 的字节码里读出来的（`defaultTransitionSpec` 里 `sipush 700`，进出一个），起因是转场体感慢到值得去量。**对照实测**：同一台模拟器、同一段导航，700ms 下点击后 0.20s 截到的是混合帧（离新旧两端都远），200ms 下同一时刻已经落定。没有改成位移推入：播放器会自己把画面转成横屏，画面在动、转场也在动，就成了两件事同时要人看。判据与时长是 `AppNavigation.kt` 里的 `SCREEN_FADE_MS`，改一个数。
...
- **列表与网格两种视图**，工具栏按钮切换，偏好存进设置（全局而非按服务器——这是「习惯怎么看」，不是某台 NAS 的属性；浏览页与播放列表页也共用它，所以一处切换两处同步）。网格用 `GridCells.Adaptive(100.dp)`：同一份代码在 411dp 的手机上给三列、800dp 的平板上给七列，不必去问屏幕宽度。这个值改过两次，而第二次是因为**第一次并没有做到它想做的事**：`Adaptive` 的列数是 `(宽 - 24 + 12) / (GRID_MIN_CELL + 12)`（24 是两侧 12dp 内边距，12 是格间距），所以**三列的门槛是 `3 × (GRID_MIN_CELL + 12) + 12`**。150.dp 是 498dp，110.dp 是 378dp——两个都高于 360dp 这个最常见的手机宽度，于是「手机上三列」在一加 Ace 5 上仍是两列（1264px 宽、densityDpi 560 → 361dp，`348 / 122 = 2.85`）。**这是模拟器测不出来的那类错误**：模拟器 411dp，`399 / 122 = 3.27`，一直是三列。要复现手机那一档，把模拟器的密度改掉即可——`adb shell wm density 479` 让 1080px 变成 361dp，与一加 Ace 5 同宽（`wm density reset` 还原）。**凡是「手机上几列」这类问题，都要先把模拟器压到 360dp 再看**，否则默认的 411dp 会把两列的问题一直盖住。100.dp 把门槛压到 348dp，360dp 的手机给 `348 / 112 = 3.1` 列，留了约 12dp 余量；104.dp 也够但正好卡在 `348 / 116 = 3.0`，宽度差 1dp 就掉回两列，不能取。原为 150.dp 的理由是竖握手机上的三列会「小到读不清」，那条理由对文字成立、对缩略图不成立：104dp 的一帧仍看得出是哪部片。代价与上次同向——大屏再多一列（800dp 平板六变七）。两种视图共用同一个缩略图组件，所以「只给可见项取帧」这条规则只有一处实现。
...
**顶栏跟着控制条一起出现、一起消失，靠轮询控制视图的 `visibility`。** 控制条自动隐藏时整个 `PlayerControlView` 会走到 `GONE`——但 `PlayerView` 提供的那两个 visibility 监听器**在这条路径上一次都不会被调用**（两个重载都接过，日志里什么都没有），它们只在**用户手动隐藏**时才响，这正是它看起来时好时坏的原因。所以顶栏读的是那个视图的 `visibility`，每 100 ms 一次。等回调的写法会让标题栏永远赖着不走，而下面的控制条早就没了。

找那个视图要**按类型**在 `PlayerView` 的子视图里找，不能按 id：它自己的 id 属于 Media3。中途还试错过两次——先猜「只改 alpha」（实测 alpha 一直是 1），再猜「靠 `translationY` 滑出屏幕」（实测 `translationY` 是 0，那个位移动画是「隐藏进度条」用的）——直到把候选值全部打出来，才看到 `v=8`。**这类问题不要推理，要打印。**
