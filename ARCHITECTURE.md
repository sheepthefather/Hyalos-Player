...
- **列表与网格两种视图**，工具栏按钮切换，偏好存进设置（全局而非按服务器——这是「习惯怎么看」，不是某台 NAS 的属性；浏览页与播放列表页也共用它，所以一处切换两处同步）。网格用 `GridCells.Adaptive(150.dp)`：同一份代码在竖握手机上给两列、平板上给五列，不必去问屏幕宽度。两种视图共用同一个缩略图组件，所以「只给可见项取帧」这条规则只有一处实现。
